...
class BasicResponse(Resource):
    ...
    def __init__(self, factory):
        ...
        with open(os.path.join(Heartbeat.resource_dir(), "index.html")) as f:
            text = f.read()
    ...
    def render(self, request):
        logdata = {"_TYPE": "HEARTBEAT"}
        logtype = self.factory.logger.LOG_HEARTBEAT
...
class Heartbeat(CanaryService):
    NAME = "heartbeat"

    def __init__(self, config=None, logger=None):
        CanaryService.__init__(self, config=config, logger=logger)
        self.port = int(config.getVal("heartbeat.port", default=1210))
        self.staticdir = os.path.join(Heartbeat.resource_dir(), "static")
        self.banner = config.getVal(
            "heartbeat.banner", default="Apache/2.4.66 (Ubuntu)"
        )
